...
describe('ListingForm kit details (player/club/season/competition)', () => {
  ...
  it('typing into player and submitting sends the field to the API', async () => {
    renderPage()
  ...
  it('typing into club, season, competition all flow through to the API', async () => {
    renderPage()
    await fillRequiredFields()
  ...
  it('edit flow: GET prefills player/club/season/competition into the inputs', async () => {
    ...
    expect((screen.getByLabelText('player') as HTMLInputElement).value).toBe('Lionel Messi')
    expect((screen.getByLabelText('club') as HTMLInputElement).value).toBe('Argentina')
    expect((screen.getByLabelText('season') as HTMLInputElement).value).toBe('2014')
    expect((screen.getByLabelText('competition') as HTMLInputElement).value).toBe('FIFA World Cup')
  })
})
