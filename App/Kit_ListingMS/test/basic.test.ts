import {test, expect} from 'vitest'
import supertest from 'supertest'
import {server} from './setup';

export interface Member {
  email: string
  password: string
  name: string
}

export const anna = {
  email: 'anna@books.com',
  password: 'annaadmin',
  name: "Anna Admin",
}

const tommy = {
  email: "tommy@books.com",
  password: "tommytimekeeper",
  name: "Tommy Timekeeper",
}
const tommyPosts: string[] = []

const timmy = {
  email: "timmy@books.com",
  password: "timmyteaboy",
  name: "Timmy Teaboy",
}
let timmyId: string
const timmyPosts: string[] = []

const terry = {
  email: "terry@books.com",
  password: "terrytroublemaker",
  name: "Terry Troublemaker",
}

const post = {
  content: 'Some old guff',
  image:
    'https://communications.ucsc.edu/wp-content/uploads/2016/11/ucsc-seal.jpg',
}

async function loginAs(member: Member): Promise<string | undefined> {
  let authToken: string | undefined
  await supertest(server)
    .post('/api/v0/login')
    .send({ email: member.email, password: member.password })
    .then((res) => {
      authToken = res.body.authToken
    })
  return authToken
}

test('Anna login', async () => {
  const authToken = await loginAs(anna)
  expect(authToken).not.equals(undefined)
})

test('Anna creates Tommy, Timmy and Terry', async () => {
  const authToken = await loginAs(anna)
  await supertest(server)
    .post('/api/v0/member/')
    .set('Authorization', 'Bearer ' + authToken)
    .send(tommy)
    .expect(201)
  await supertest(server)
    .post('/api/v0/member/')
    .set('Authorization', 'Bearer ' + authToken)
    .send(timmy)
    .expect(201)
    .then((res) => {
      timmyId = res.body.id
    })
  await supertest(server)
    .post('/api/v0/member/')
    .set('Authorization', 'Bearer ' + authToken)
    .send(terry)
    .expect(201)
})

async function makePost(authToken: string | undefined) {
  await supertest(server)
    .post('/api/v0/post/')
    .set('Authorization', 'Bearer ' + authToken)
    .send(post)
    .expect(201)
    .then((res) => {
      tommyPosts.push(res.body.id)
    })
}

test('Tommy makes two posts', async () => {
  const authToken = await loginAs(tommy)
  await makePost(authToken)
  await makePost(authToken)
})

async function seesPosts(authToken: string | undefined, numPosts: number) {
  await supertest(server)
    .get('/api/v0/post?page=1')
    .set('Authorization', 'Bearer ' + authToken)
    .expect(200)
    .then((res) => {
      expect(res.body.length).toBe(numPosts)
    })
}

test("Timmy cannot see Tommy's posts", async () => {
  const authToken = await loginAs(timmy)
  await seesPosts(authToken, 0)
})

test('Tommy sends Timmy a friend request', async () => {
  const authToken = await loginAs(tommy)
  await supertest(server)
    .post('/api/v0/request/' + timmyId)
    .set('Authorization', 'Bearer ' + authToken)
    .expect(201)
})

test("Timmy cannot see Tommy's posts until he accepts request", async () => {
  const authToken = await loginAs(timmy)
  await seesPosts(authToken, 0)
})

test("Timmy accepts Tommy's friend request", async () => {
  const authToken = await loginAs(timmy)
  let fid = ''
  await supertest(server)
    .get('/api/v0/request/inbound')
    .set('Authorization', 'Bearer ' + authToken)
    .expect(200)
    .then((res) => {
      fid = res.body[0].id
    })
  await supertest(server)
    .put('/api/v0/request/' + fid)
    .set('Authorization', 'Bearer ' + authToken)
    .expect(200)
})

test('Tommy now has no outbound requests', async () => {
  const authToken = await loginAs(tommy)
  await supertest(server)
    .get('/api/v0/request/outbound')
    .set('Authorization', 'Bearer ' + authToken)
    .expect(200)
    .then((res) => {
      expect(res.body.length).toBe(0)
    })
})

test("Timmy can now see Tommy's posts", async () => {
  const authToken = await loginAs(timmy)
  await seesPosts(authToken, 2)
})

test("Terry cannot see Tommy's posts", async () => {
  const authToken = await loginAs(terry)
  await seesPosts(authToken, 0)
})

test('Timmy makes a post', async () => {
  const authToken = await loginAs(timmy)
  await supertest(server)
    .post('/api/v0/post/')
    .set('Authorization', 'Bearer ' + authToken)
    .send(post)
    .expect(201)
    .then((res) => {
      timmyPosts.push(res.body.id)
    })
})

async function seesFriendPosts(authToken: string | undefined) {
  await supertest(server)
    .get('/api/v0/post?page=1')
    .set('Authorization', 'Bearer ' + authToken)
    .expect(200)
    .then((res) => {
      expect(res.body.length).toBe(3)
      expect(res.body[0].id).toBe(timmyPosts[0])
      expect(res.body[1].id).toBe(tommyPosts[1])
      expect(res.body[2].id).toBe(tommyPosts[0])
    })
}

test("Tommy can see Timmy's post and his own", async () => {
  const authToken = await loginAs(tommy)
  await seesFriendPosts(authToken)
})

test("Timmy can see Tommy's posts and his own", async () => {
  const authToken = await loginAs(timmy)
  await seesFriendPosts(authToken)
})

test("Terry cannot see Timmy's or Tommy's posts", async () => {
  const authToken = await loginAs(terry)
  await supertest(server)
    .get('/api/v0/post?page=1')
    .set('Authorization', 'Bearer ' + authToken)
    .expect(200)
    .then((res) => {
      expect(res.body.length).toBe(0)
    })
})

test('Tommy no longer wants Timmy as a friend', async () => {
  const authToken = await loginAs(tommy)
  await supertest(server)
    .delete('/api/v0/friend/' + timmyId)
    .set('Authorization', 'Bearer ' + authToken)
    .expect(200)
    .then((res) => {
      expect(res.body.name).toBe(timmy.name)
    })
})

test('Timmy can now only see his own post', async () => {
  const authToken = await loginAs(timmy)
  await supertest(server)
    .get('/api/v0/post?page=1')
    .set('Authorization', 'Bearer ' + authToken)
    .expect(200)
    .then((res) => {
      expect(res.body.length).toBe(1)
      expect(res.body[0].id).toBe(timmyPosts[0])
    })
})

test('Tommy can now only see his own posts', async () => {
  const authToken = await loginAs(tommy)
  await supertest(server)
    .get('/api/v0/post?page=1')
    .set('Authorization', 'Bearer ' + authToken)
    .expect(200)
    .then((res) => {
      expect(res.body.length).toBe(2)
      expect(res.body[0].id).toBe(tommyPosts[1])
      expect(res.body[1].id).toBe(tommyPosts[0])
    })
})
