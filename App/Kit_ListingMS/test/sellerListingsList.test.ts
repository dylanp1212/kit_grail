...
describe('GET /api/v0/seller/listings', () => {
  ...
  it('returns only listings owned by the authenticated seller', async () => {
    await supertest(server)
      ...
      .send({ title: 'Mine A', description: 'd', size: 'large', colors: ['red'], price: 10 })
    await supertest(server)
      .post('/api/v0/seller/listings')
      .set('Authorization', `Bearer ${seller.key}`)
      .send({ title: 'Mine B', description: 'd', size: 'large', colors: ['red'], price: 20 })
  ...
  it('filters by search query', async () => {
    await supertest(server)
      .post('/api/v0/seller/listings')
      .set('Authorization', `Bearer ${seller.key}`)
      .send({ title: 'Vintage Argentina', description: 'd', size: 'large', colors: ['blue'], price: 50 })
    await supertest(server)
      .post('/api/v0/seller/listings')
      .set('Authorization', `Bearer ${seller.key}`)
      .send({ title: 'Modern Brazil', description: 'd', size: 'large', colors: ['yellow'], price: 60 })
