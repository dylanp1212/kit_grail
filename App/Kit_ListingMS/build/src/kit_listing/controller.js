...
let ListingController = class ListingController extends tsoa_1.Controller {
    ...
    async createNewKitListing(newListing, request) {
        if (!request.user?.id) {
            this.setStatus(401);
            return undefined;
        }
        ...
        this.setStatus(201);
        return new service_1.ListingService().createNewKitListing(withSeller);
    }
    async editKitListing(listing, request, id) {
        if (!request.user?.id) {
            this.setStatus(401);
            return undefined;
        }
        this.setStatus(201);
        return await new service_1.ListingService().editKitListing(id, request.user.id, listing);
    }
};
...
__decorate([
    ...
    __metadata("design:returntype", Promise)
], ListingController.prototype, "editKitListing", null);
