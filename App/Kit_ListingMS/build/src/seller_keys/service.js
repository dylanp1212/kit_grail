...
class KeyManagementService {
    async create(sellerId, req) {
        ...
        const res = await db_1.pool.query(`INSERT INTO api_key(seller, data)
       VALUES ($1, $2::jsonb)
       RETURNING id`, [sellerId, JSON.stringify({ prefix, hash, label: req.label, created_at })]);
        return { id: res.rows[0].id, prefix, plaintext, label: req.label, created_at };
    }
    async list(sellerId) {
        const res = await db_1.pool.query(`SELECT id, data FROM api_key
       WHERE seller = $1
